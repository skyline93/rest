...
func LoadKey(ctx context.Context, s *Repository, id rest.ID) (k *Key, err error) {
	data, err := s.LoadRaw(ctx, rest.KeyFile, id)
	if err != nil {
		return nil, err
	}
...
func AddKey(ctx context.Context, s *Repository, password, username, hostname string, template *crypto.Key) (*Key, error) {
	...
	if params == nil {
		...
		params = &p
		log.Infof("calibrated KDF parameters are %v", p)
	}
...
func OpenKey(ctx context.Context, s *Repository, id rest.ID, password string) (*Key, error) {
	k, err := LoadKey(ctx, s, id)
	if err != nil {
		log.Printf("LoadKey(%v) returned error %v", id.String(), err)
		return nil, err
	}
	...
	if err != nil {
		log.Printf("Unmarshal() returned error %v", err)
		return nil, errors.Wrap(err, "Unmarshal")
	}
	k.id = id
...
func SearchKey(ctx context.Context, s *Repository, password string, maxKeys int, keyHint string) (k *Key, err error) {
	...
	if len(keyHint) > 0 {
		id, err := rest.Find(ctx, s, rest.KeyFile, keyHint)

		if err == nil {
			key, err := OpenKey(ctx, s, id, password)

			if err == nil {
				log.Infof("successfully opened hinted key %v", id)
				return key, nil
			}

			log.Infof("could not open hinted key %v", id)
		} else {
			log.Infof("Could not find hinted key %v", keyHint)
		}
	}
	...
	err = s.List(listCtx, rest.KeyFile, func(id rest.ID, _ int64) error {
		checked++
		if maxKeys > 0 && checked > maxKeys {
			return ErrMaxKeysReached
		}

		log.Infof("trying key %q", id.String())
		key, err := OpenKey(ctx, s, id, password)
		if err != nil {
			log.Infof("key %v returned error %v", id.String(), err)
		...
		log.Infof("successfully opened key %v", id.String())
		k = key
		cancel()
		return nil
	})
