...
var entrySize = uint(binary.Size(rest.BlobType(0)) + 2*headerLengthSize + len(rest.ID{}))
var plainEntrySize = uint(binary.Size(rest.BlobType(0)) + headerLengthSize + len(rest.ID{}))
...
func (p *Packer) Add(t rest.BlobType, id rest.ID, data []byte, uncompressedLength int) (int, error) {
	p.m.Lock()
	defer p.m.Unlock()

	c := rest.Blob{BlobHandle: rest.BlobHandle{Type: t, ID: id}}
...
func CalculateEntrySize(blob rest.Blob) int {
...
func makeHeader(blobs []rest.Blob) ([]byte, error) {
	buf := make([]byte, 0, len(blobs)*int(entrySize))

	for _, b := range blobs {
		switch {
		case b.Type == rest.DataBlob && b.UncompressedLength == 0:
			buf = append(buf, 0)
		case b.Type == rest.TreeBlob && b.UncompressedLength == 0:
			buf = append(buf, 1)
		case b.Type == rest.DataBlob && b.UncompressedLength != 0:
			buf = append(buf, 2)
		case b.Type == rest.TreeBlob && b.UncompressedLength != 0:
...
func verifyHeader(k *crypto.Key, header []byte, expected []rest.Blob) error {
...
func List(k *crypto.Key, rd io.ReaderAt, size int64) (entries []rest.Blob, hdrSize uint32, err error) {
	...
	entries = make([]rest.Blob, 0, uint(len(buf))/plainEntrySize)
...
func parseHeaderEntry(p []byte) (b rest.Blob, size uint, err error) {
	l := uint(len(p))
	size = plainEntrySize
	if l < plainEntrySize {
	...
	switch tpe {
	case 0, 2:
		b.Type = rest.DataBlob
	case 1, 3:
		b.Type = rest.TreeBlob
	default:
...
func readRecords(rd io.ReaderAt, size int64, bufsize int) ([]byte, int, error) {
	...
	log.Infof("header length: %v", hlen)
