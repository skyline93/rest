...
func (r *Repository) LoadRaw(ctx context.Context, t rest.FileType, id rest.ID) (buf []byte, err error) {
	...
	if h.Type != backend.ConfigFile && id != rest.Hash(buf) {
		// if r.Cache != nil {
		...
		if err == nil && id != rest.Hash(buf) {
			...
			return buf, fmt.Errorf("LoadRaw(%v): %w", h, rest.ErrInvalidData)
		}
	}
