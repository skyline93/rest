...
func (node *Node) CreateAt(ctx context.Context, path string, repo Repository) error {
	log.Printf("create node %v at %v", node.Name, path)
...
func (node Node) RestoreMetadata(path string) error {
	...
	if err != nil {
		log.Printf("restoreMetadata(%s) error %v", path, err)
	}
...
func (node Node) restoreMetadata(path string) error {
	...
	if err := lchown(path, int(node.UID), int(node.GID)); err != nil {
		...
		if os.Geteuid() > 0 && os.IsPermission(err) {
			log.Printf("not running as root, ignoring lchown permission error for %v: %v",
				path, err)
		} else {
	...
	if err := node.RestoreTimestamps(path); err != nil {
		log.Printf("error restoring timestamps for dir %v: %v", path, err)
		if firsterr != nil {
			firsterr = err
		}
	}
	...
	if err := node.restoreExtendedAttributes(path); err != nil {
		log.Printf("error restoring extended attributes for %v: %v", path, err)
		if firsterr != nil {
			firsterr = err
		}
	}
...
func (node Node) sameExtendedAttributes(other Node) bool {
	...
	for _, attr := range other.ExtendedAttributes {
		...
		if !ok {
			...
			log.Printf("other node has attribute %v, which is not present in node", attr.Name)
			return false
		...
		if !bytes.Equal(v.value, attr.Value) {
			...
			log.Printf("attribute %v has different value", attr.Name)
			return false
		}
	...
	for name, v := range attributes {
		if !v.present {
			log.Printf("attribute %v not present in other node", name)
			return false
		}
	}
...
func (node *Node) fillExtendedAttributes(path string) error {
	...
	log.Printf("fillExtendedAttributes(%v) %v %v", path, xattrs, err)
	if err != nil {
		return err
	}
