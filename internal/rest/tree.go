...
func (t *Tree) Equals(other *Tree) bool {
	if len(t.Nodes) != len(other.Nodes) {
		log.Printf("tree.Equals(): trees have different number of nodes")
		return false
	}

	for i := 0; i < len(t.Nodes); i++ {
		if !t.Nodes[i].Equals(*other.Nodes[i]) {
			log.Printf("tree.Equals(): node %d is different:", i)
			log.Printf("  %#v", t.Nodes[i])
			log.Printf("  %#v", other.Nodes[i])
			return false
		}
	}
