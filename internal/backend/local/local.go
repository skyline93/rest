...
import (
	...
	"github.com/skyline93/rest/internal/backend/util"
...
func open(ctx context.Context, cfg Config) (*Local, error) {
	...
	log.Infof("using (%03O file, %03O dir) permissions", m.File, m.Dir)
...
func Create(ctx context.Context, cfg Config) (*Local, error) {
	log.Infof("create local backend at %v (layout %q)", cfg.Path, cfg.Layout)
...
func (b *Local) List(ctx context.Context, t backend.FileType, fn func(backend.FileInfo) error) (err error) {
	...
	if b.IsNotExist(err) {
		log.Infof("ignoring non-existing directory")
		return nil
	}
...
func (b *Local) Save(_ context.Context, h backend.Handle, rd backend.RewindReader) (err error) {
	...
	if b.IsNotExist(err) {
		log.Infof("error %v: creating dir", err)

		// error is caused by a missing directory, try to create it
		mkdirErr := fs.MkdirAll(dir, b.Modes.Dir)
		if mkdirErr != nil {
			log.Infof("error creating dir %v: %v", dir, mkdirErr)
		} else {
			// try again
			f, err = tempFile(dir, tmpname)
		}
	...
	if size := rd.Length(); size > 0 {
		if err := fs.PreallocateFile(f, size); err != nil {
			log.Infof("Failed to preallocate %v with size %v: %v", finalname, size, err)
		}
	}
