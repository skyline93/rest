...
import (
	...
	"github.com/skyline93/rest/internal/backend/layout"
...
func open(ctx context.Context, cfg Config) (*Local, error) {
	...
	fi, err := fs.Stat(l.Filename(rest.Handle{Type: rest.ConfigFile}))
	m := backend.DeriveModesFromFileInfo(fi, err)
	log.Printf("using (%03O file, %03O dir) permissions", m.File, m.Dir)
...
func Create(ctx context.Context, cfg Config) (*Local, error) {
	log.Printf("create local backend at %v (layout %q)", cfg.Path, cfg.Layout)
	...
	_, err = fs.Lstat(be.Filename(rest.Handle{Type: rest.ConfigFile}))
	if err == nil {
...
func (b *Local) Save(_ context.Context, h rest.Handle, rd rest.RewindReader) (err error) {
	...
	if b.IsNotExist(err) {
		log.Printf("error %v: creating dir", err)

		// error is caused by a missing directory, try to create it
		mkdirErr := fs.MkdirAll(dir, b.Modes.Dir)
		if mkdirErr != nil {
			log.Printf("error creating dir %v: %v", dir, mkdirErr)
		} else {
			// try again
			f, err = tempFile(dir, tmpname)
		}
	...
	if size := rd.Length(); size > 0 {
		if err := fs.PreallocateFile(f, size); err != nil {
			log.Printf("Failed to preallocate %v with size %v: %v", finalname, size, err)
		}
	}
...
func (b *Local) List(ctx context.Context, t rest.FileType, fn func(rest.FileInfo) error) (err error) {
	...
	if b.IsNotExist(err) {
		log.Println("ignoring non-existing directory")
		return nil
	}
...
func visitDirs(ctx context.Context, dir string, fn func(rest.FileInfo) error) error {
	d, err := fs.Open(dir)
	if err != nil {
		return err
	}
...
func visitFiles(ctx context.Context, dir string, fn func(rest.FileInfo) error, ignoreNotADirectory bool) error {
	d, err := fs.Open(dir)
	if err != nil {
		return err
	}
...
// given offset.
func (b *Local) Load(ctx context.Context, h rest.Handle, length int, offset int64, fn func(rd io.Reader) error) error {
	return backend.DefaultLoad(ctx, h, length, offset, b.openReader, fn)
}

func (b *Local) openReader(_ context.Context, h rest.Handle, length int, offset int64) (io.ReadCloser, error) {
...
func (b *Local) Remove(_ context.Context, h rest.Handle) error {
	fn := b.Filename(h)
...
func (b *Local) Stat(_ context.Context, h rest.Handle) (rest.FileInfo, error) {
	fi, err := fs.Stat(b.Filename(h))
	if err != nil {
		return rest.FileInfo{}, errors.WithStack(err)
	}

	return rest.FileInfo{Size: fi.Size(), Name: h.Name}, nil
}
