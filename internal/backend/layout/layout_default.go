...
var defaultLayoutPaths = map[rest.FileType]string{
	rest.PackFile:     "data",
...
func (l *DefaultLayout) Dirname(h rest.Handle) string {
	p := defaultLayoutPaths[h.Type]

	if h.Type == rest.PackFile && len(h.Name) > 2 {
		p = l.Join(p, h.Name[:2]) + "/"
	}
...
func (l *DefaultLayout) Filename(h rest.Handle) string {
	name := h.Name
	if h.Type == rest.ConfigFile {
...
func (l *DefaultLayout) Paths() (dirs []string) {
	...
	for i := 0; i < 256; i++ {
		...
		dirs = append(dirs, l.Join(l.Path, defaultLayoutPaths[rest.PackFile], subdir))
	}
...
func (l *DefaultLayout) Basedir(t rest.FileType) (dirname string, subdirs bool) {
	if t == rest.PackFile {
		subdirs = true
	}
