...
type Layout interface {
	Filename(rest.Handle) string
	Dirname(rest.Handle) string
	Basedir(rest.FileType) (dir string, subdirs bool)
	Paths() []string
	Name() string
}
...
var backendFilenameLength = len(rest.ID{}) * 2
...
func DetectLayout(ctx context.Context, repo Filesystem, dir string) (Layout, error) {
	log.Printf("detect layout at %v", dir)
	if repo == nil {
		repo = &LocalFilesystem{}
	}

	// key file in the "keys" dir (DefaultLayout)
	foundKeysFile, err := hasBackendFile(ctx, repo, repo.Join(dir, defaultLayoutPaths[rest.KeyFile]))
	if err != nil {
		return nil, err
	}

	// key file in the "key" dir (S3LegacyLayout)
	foundKeyFile, err := hasBackendFile(ctx, repo, repo.Join(dir, s3LayoutPaths[rest.KeyFile]))
	if err != nil {
		return nil, err
	}

	if foundKeysFile && !foundKeyFile {
		log.Printf("found default layout at %v", dir)
	...
	if foundKeyFile && !foundKeysFile {
		log.Printf("found s3 layout at %v", dir)
	...
	log.Printf("layout detection failed")
...
func ParseLayout(ctx context.Context, repo Filesystem, layout, defaultLayout, path string) (l Layout, err error) {
	log.Printf("parse layout string %q for backend at %v", layout, path)
	switch layout {
	case "default":
		l = &DefaultLayout{
			Path: path,
	...
	case "":
		...
		if errors.Is(err, ErrLayoutDetectionFailed) && defaultLayout != "" {
			log.Printf("error: %v, use default layout %v", err, defaultLayout)
		...
		log.Printf("layout detected: %v", l)
	default:
