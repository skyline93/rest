...
type SaveBlobFn func(context.Context, rest.BlobType, *Buffer, string, func(res SaveBlobResponse))
...
type FileSaver struct {
	...
	NodeFromFileInfo func(snPath, filename string, fi os.FileInfo, ignoreXattrListError bool) (*rest.Node, error)
}
...
type CompleteFunc func(*rest.Node, ItemStats)
...
func NewFileSaver(ctx context.Context, wg *errgroup.Group, save SaveBlobFn, pol chunker.Pol, fileWorkers, blobWorkers uint) *FileSaver {
	...
	log.Infof("new file saver with %v file workers and %v blob workers", fileWorkers, blobWorkers)
...
func (s *FileSaver) saveFile(ctx context.Context, chnker *chunker.Chunker, snPath string, target string, f fs.File, fi os.FileInfo, start func(), finishReading func(), finish func(res futureNodeResult)) {
	...
	for {
		...
		lock.Lock()
		node.Content = append(node.Content, rest.ID{})
		lock.Unlock()

		s.saveBlob(ctx, rest.DataBlob, buf, target, func(sbr SaveBlobResponse) {
			lock.Lock()
			if !sbr.known {
				fnr.stats.DataBlobs++
...
func (s *FileSaver) Save(ctx context.Context, snPath string, target string, file fs.File, fi os.FileInfo, start func(), completeReading func(), complete CompleteFunc) FutureNode {
	...
	case s.ch <- job:
	case <-ctx.Done():
		log.Debugf("not sending job, context is cancelled: %v", ctx.Err())
		_ = file.Close()
		close(ch)
	}
