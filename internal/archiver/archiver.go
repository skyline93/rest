...
func New(repo rest.Repository, fs fs.FS, opts Options) *Archiver {
	arch := &Archiver{
		Repo:         repo,
...
func (arch *Archiver) error(item string, err error) error {
	...
	if err != errf {
		log.Printf("item %v: error was filtered by handler, before: %q, after: %v", item, err, errf)
	}
	return errf
}

// nodeFromFileInfo returns the rest node from an os.FileInfo.
func (arch *Archiver) nodeFromFileInfo(snPath, filename string, fi os.FileInfo) (*rest.Node, error) {
	node, err := rest.NodeFromFileInfo(filename, fi)
	if !arch.WithAtime {
...
func (arch *Archiver) loadSubtree(ctx context.Context, node *rest.Node) (*rest.Tree, error) {
	...
	if err != nil {
		log.Printf("unable to load tree %v: %v", node.Subtree.Str(), err)
...
func (arch *Archiver) wrapLoadTreeError(id rest.ID, err error) error {
	if arch.Repo.Index().Has(rest.BlobHandle{ID: id, Type: rest.TreeBlob}) {
...
func (arch *Archiver) SaveDir(ctx context.Context, snPath string, dir string, fi os.FileInfo, previous *rest.Tree, complete CompleteFunc) (d FutureNode, err error) {
	...
	for _, name := range names {
		...
		if ctx.Err() != nil {
			log.Printf("context has been cancelled, aborting")
...
func (arch *Archiver) allBlobsPresent(previous *rest.Node) bool {
	...
	for _, id := range previous.Content {
		if !arch.Repo.Index().Has(rest.BlobHandle{ID: id, Type: rest.DataBlob}) {
			return false
		}
	}
	return true
...
func (arch *Archiver) Save(ctx context.Context, snPath, target string, previous *rest.Node) (fn FutureNode, excluded bool, err error) {
	...
	log.Printf("%v target %q, previous %v", snPath, target, previous)
	...
	if !arch.SelectByName(abstarget) {
		log.Printf("%v is excluded by path", target)
	...
	if err != nil {
		log.Printf("lstat() for %v returned error: %v", target, err)
	...
	if !arch.Select(abstarget, fi) {
		log.Printf("%v is excluded", target)
		return FutureNode{}, true, nil
	}

	switch {
	case fs.IsRegularFile(fi):
		log.Printf("  %v regular file", target)
		...
		if previous != nil && !fileChanged(fi, previous, arch.ChangeIgnoreFlags) {
			if arch.allBlobsPresent(previous) {
				log.Printf("%v hasn't changed, using old list of blobs", target)
			...
			log.Printf("%v hasn't changed, but contents are missing!", target)
		...
		if err != nil {
			log.Printf("Openfile() for %v returned error: %v", target, err)
			err = arch.error(abstarget, err)
			if err != nil {
				return FutureNode{}, false, errors.WithStack(err)
			}
		...
		if err != nil {
			log.Printf("stat() on opened file %v returned error: %v", target, err)
			_ = file.Close()
			err = arch.error(abstarget, err)
			if err != nil {
				return FutureNode{}, false, errors.WithStack(err)
	...
	case fi.IsDir():
		...
		if err != nil {
			log.Printf("SaveDir for %v returned error: %v", snPath, err)
			return FutureNode{}, false, err
		}

	case fi.Mode()&os.ModeSocket > 0:
		log.Printf("  %v is a socket, ignoring", target)
		return FutureNode{}, true, nil

	default:
		log.Printf("  %v other", target)
	...
	log.Printf("return after %.3f", time.Since(start).Seconds())
...
func (arch *Archiver) SaveTree(ctx context.Context, snPath string, atree *Tree, previous *rest.Tree, complete CompleteFunc) (FutureNode, int, error) {
	...
	if snPath != "/" {
		...
		log.Printf("%v, dir node data loaded from %v", snPath, atree.FileInfoPath)
	...
	log.Printf("%v (%v nodes), parent %v", snPath, len(atree.Nodes), previous)
...
func (arch *Archiver) loadParentTree(ctx context.Context, sn *rest.Snapshot) *rest.Tree {
	...
	if sn.Tree == nil {
		log.Printf("snapshot %v has empty tree %v", *sn.ID())
		return nil
	}

	log.Printf("load parent tree %v", *sn.Tree)
	tree, err := rest.LoadTree(ctx, arch.Repo, *sn.Tree)
	if err != nil {
		log.Printf("unable to load tree %v: %v", *sn.Tree, err)
...
func (arch *Archiver) Snapshot(ctx context.Context, targets []string, opts SnapshotOptions) (*rest.Snapshot, rest.ID, error) {
	cleanTargets, err := resolveRelativeTargets(arch.FS, targets)
	if err != nil {
		return nil, rest.ID{}, err
	}
	...
	wgUp.Go(func() error {
		...
		wg.Go(func() error {
			...
			log.Printf("starting snapshot")
			fn, nodeCount, err := arch.SaveTree(wgCtx, "/", atree, arch.loadParentTree(wgCtx, opts.ParentSnapshot), func(n *rest.Node, is ItemStats) {
		...
		if err != nil {
			log.Printf("error while saving tree: %v", err)
			return err
		}
